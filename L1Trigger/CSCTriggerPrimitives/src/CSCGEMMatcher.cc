...
int CSCGEMMatcher::matchedClusterDistES(const CSCCLCTDigi& clct,
  ...
  if (station_ == 1) {  // GE1/1
    if (endcap_ == 1)  // Positive endcap
      alignCorrection = isLayer2 ? lookupTableME11ILT_->GEM_align_corr_es_ME11_positive_endcap(chamber_, cl.roll2())
                                 : lookupTableME11ILT_->GEM_align_corr_es_ME11_positive_endcap(chamber_, cl.roll1());
    else // Negative endcap
      alignCorrection = isLayer2 ? lookupTableME11ILT_->GEM_align_corr_es_ME11_negative_endcap(chamber_, cl.roll2())
                                 : lookupTableME11ILT_->GEM_align_corr_es_ME11_negative_endcap(chamber_, cl.roll1());
  } else { // GE2/1
    if (endcap_ == 1)  // Positive endcap
      alignCorrection = isLayer2 ? lookupTableME21ILT_->GEM_align_corr_es_ME21_positive_endcap(chamber_, cl.roll2())
                                 : lookupTableME21ILT_->GEM_align_corr_es_ME21_positive_endcap(chamber_, cl.roll1());
    else // Negative endcap
      alignCorrection = isLayer2 ? lookupTableME21ILT_->GEM_align_corr_es_ME21_negative_endcap(chamber_, cl.roll2())
  ...
  int eighthStripDiff = cl_es + alignCorrection - clct.getKeyStrip(8);
    // std::cout << "CSCGEMMatcher.cc " << "Endcap = " << endcap_
    //                                 << " Station = " << station_ 
    //                                 << " Chamber = " << chamber_ 
    //                                 << " Correction = " << alignCorrection 
    //                                 << " cl_es = " << cl_es
    //                                 << " eighthStripDiff = " << eighthStripDiff <<std::endl;
