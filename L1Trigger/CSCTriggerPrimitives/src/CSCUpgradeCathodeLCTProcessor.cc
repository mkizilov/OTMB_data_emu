  ...
  while (start_bx < stop_bx) {
    ...
    if (pre_trig) {
      ...
      if (hits_in_time) {
        ...
        markPreTriggerZone(pretrig_zone, first_bx);
...
void CSCUpgradeCathodeLCTProcessor::markPreTriggerZone(
    bool pretrig_zone[CSCConstants::MAX_NUM_HALF_STRIPS_RUN2_TRIGGER], int first_bx) const {
  ...
  for (int hstrip = 0; hstrip < CSCConstants::MAX_NUM_HALF_STRIPS_RUN2_TRIGGER; hstrip++) {
    if (ispretrig_[hstrip] && !busyMap_[hstrip][first_bx]) {
      ...
      if (isME11_) {
        if (hstrip < CSCConstants::NUM_STRIPS_ME1B*2 && max_hs >= CSCConstants::NUM_STRIPS_ME1B*2)
              max_hs = CSCConstants::NUM_STRIPS_ME1B*2 -1;
        if (hstrip >= CSCConstants::NUM_STRIPS_ME1B*2 && min_hs < CSCConstants::NUM_STRIPS_ME1B*2)
              min_hs = CSCConstants::NUM_STRIPS_ME1B*2;
      }
...
void CSCUpgradeCathodeLCTProcessor::markBusyZone(const int bx) {
  for (int hstrip = stagger[CSCConstants::KEY_CLCT_LAYER - 1]; hstrip < numHalfStrips_; hstrip++) {
    ...
    if (ispretrig_[hstrip]) {
      ...
      if (isME11_) {
        if (hstrip < CSCConstants::NUM_STRIPS_ME1B*2 && max_hstrip >= CSCConstants::NUM_STRIPS_ME1B*2)
              max_hstrip = CSCConstants::NUM_STRIPS_ME1B*2 -1;
        if (hstrip >= CSCConstants::NUM_STRIPS_ME1B*2 && min_hstrip < CSCConstants::NUM_STRIPS_ME1B*2)
              min_hstrip = CSCConstants::NUM_STRIPS_ME1B*2;
      }
